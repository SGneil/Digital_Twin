(() => {
    ...
    async function loadUserInfo() {
        ...
        try {
            ...
            if (data.success && data.data.life_review) {
                // 如果成功獲取到 life_review，解析並填充表單
                const life_review = JSON.parse(data.data.life_review);
                fillForm(life_review);
            } else {
                // 如果沒有獲取到 life_review，執行對話摘要
                console.log("沒有用戶資料或 life_review 為空，執行 conversation_summary");
        ...
        data.forEach((item, index) => {
            ...
            itemContainer.innerHTML = `
                ...
                <div class="input-group">
                    <label for="content${index + 1}" class="form-label me-2">編輯內容：</label>
    ...
    async function saveFormData() {
        ...
        // 收集所有life_review項目的內容
        const life_review = Array.from(modalsContainer.children).map(child => 
            child.querySelector('textarea').value
        );

        formData.append('account_id', state.account_id);
        formData.append('topic_id', state.topic_id);
        formData.append('life_review', JSON.stringify(life_review));
