(() => {
    ...
    async function loadUserInfo() {
        ...
        try {
            ...
            if (data.success && data.data.bucket_list) {
                // 如果成功獲取到 bucket_list，解析並填充表單
                const bucket_list = JSON.parse(data.data.bucket_list);
                fillForm(bucket_list);
            } else {
                // 如果沒有獲取到 bucket_list，執行對話摘要
                console.log("沒有用戶資料或 bucket_list 為空，執行 conversation_summary");
    ...
    async function saveFormData() {
        ...
        // 收集所有夢想項目的內容
        const bucket_list = Array.from(modalsContainer.children).map(child => 
            child.querySelector('textarea').value
        );

        formData.append('account_id', state.account_id);
        formData.append('topic_id', state.topic_id);
        formData.append('bucket_list', JSON.stringify(bucket_list));
