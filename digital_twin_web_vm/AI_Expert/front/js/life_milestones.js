(() => {
    ...
    async function loadUserInfo() {
        ...
        try {
            ...
            if (data.success && data.data.life_milestones) {
                // 如果成功獲取到 life_milestones，解析並填充表單
                const life_milestones = JSON.parse(data.data.life_milestones);
                fillForm(life_milestones);
            } else {
                // 如果沒有獲取到 life_milestones，執行對話摘要
                console.log("沒有用戶資料或 life_milestones 為空，執行 conversation_summary");
    ...
    async function saveFormData() {
        ...
        // 收集所有事件項目的內容
        const life_milestones = Array.from(modalsContainer.children).map(child => 
            child.querySelector('textarea').value
        );

        formData.append('account_id', state.account_id);
        formData.append('topic_id', state.topic_id);
        formData.append('life_milestones', JSON.stringify(life_milestones));
