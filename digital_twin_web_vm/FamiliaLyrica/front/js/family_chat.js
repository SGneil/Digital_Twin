...
function get_gpt_api(text, family_picture, family_video, sovits_role, user_information_id) {
    const formData = new FormData();
    formData.append('msg', text);
    formData.append('sovits_role', sovits_role);
    formData.append('user_information_id', user_information_id);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            save_text(response, 'gpt', user_information_id).then(data => {
...
function save_text(text, role, user_information_id) {
    const formData = new FormData();
...
function get_family_video_path(type, id) {
    return new Promise((resolve, reject) => {
        ...
        $.ajax({
            url: '../end/get_family_video_path.php',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
...
btn.addEventListener('click', () => {
    ...
    if (value !== '' && value !== '思考中~') {
        ...
        const user_information_id = document.getElementById('user_information_id').dataset.value;

        // 顯示子孫文字
        showText('user2', '../../FamiliaLyrica/front/img/default_userImg.jpg', value, 'no');

        // 對話存進資料庫
        save_text(value, 'user', user_information_id);

        // 取得長輩GPT文字
        get_gpt_api(value, img_path, video_path, sovits_role, user_information_id);
    }
});
