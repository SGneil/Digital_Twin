...
parser.add_argument(
    "--temp_directory",
    type=str,
    help="Filepath of temp directory to use as raw video source",
    required=False,
)
parser.add_argument(
    "--detected_face_folder",
    type=str,
    help="Filepath of detected_face_folder to use as raw video source",
    required=False,
)
parser.add_argument(
    "--outfile",
...
parser.add_argument(
    "--wav2lip_batch_size", type=int, help="Batch size for Wav2Lip model(s)", default=4
)
...
def face_detect(images, results_file="last_detected_face.pkl"):
    ...
    for image, (rect) in tqdm_partial(
        ...
        y1 = max(0, int(rect[1]) - int(pady1))
        y2 = min(image.shape[0], int(rect[3]) + int(pady2))
        x1 = max(0, int(rect[0]) - int(padx1))
        x2 = min(image.shape[1], int(rect[2]) + int(padx2))
...
def datagen(frames, mels, detected_face_folder):
    img_batch, mel_batch, frame_batch, coords_batch = [], [], [], []
    print("\r" + " " * 100, end="\r")
    if args.box[0] == -1:
        if not args.static:
            face_det_results = face_detect(frames, detected_face_folder + "last_detected_face.pkl")  # BGR2RGB for CNN face detection
        else:
            face_det_results = face_detect([frames[0]], detected_face_folder + "last_detected_face.pkl")
    else:
    ...
    # 從頭播放
    # for i, m in enumerate(mels):
    #     idx = 0 if args.static else i % len(frames)  # 使用取模運算符計算 idx
        
    #     # 確保 idx 在 face_det_results 的範圍內循環
    #     idx = idx % len(face_det_results)
    ...
    for i, m in enumerate(mels):
        if args.static:
            ...
            # 更新索引並確保 idx 在 face_det_results 的範圍內循環
            if direction == 1:
                idx = i % len(frames)
            else:
                idx = len(frames) - 1 - (i % len(frames))
            
            # 確保 idx 在 face_det_results 的範圍內
            idx = idx % len(face_det_results)
        ...
        # 更新方向
        if not args.static:
            if direction == 1 and idx == len(frames) - 1:
                direction = -1  # 到達最後一位索引，切換為反向
            elif direction == -1 and idx == 0:
                direction = 1  # 到達第一位索引，切換為正向
...
def main(facefile, audiofile, outfile, temp_directory, detected_face_folder):
    args.img_size = 96
    frame_number = 11

    if os.path.isfile(facefile) and facefile.split(".")[1] in ["jpg", "png", "jpeg"]:
        args.static = True

    if not os.path.isfile(facefile):
        raise ValueError("--face argument must be a valid path to video/image file")

    elif facefile.split(".")[1] in ["jpg", "png", "jpeg"]:
    ...
    if not audiofile.endswith(".wav"):
        ...
        subprocess.check_call(
            [
                ...
                "-i",
                audiofile,
                temp_directory + "temp.wav",
            ]
        )
        audiofile = temp_directory + "temp.wav"
    ...
    if str(args.preview_settings) == "True":
        gen = datagen(full_frames, mel_chunks, detected_face_folder)
    else:
        gen = datagen(full_frames.copy(), mel_chunks, detected_face_folder)
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(
        ...
        if i == 0:
            ...
            out = cv2.VideoWriter(temp_directory + "result.mp4", fourcc, fps, (frame_w, frame_h))
        ...
        for p, f, c in zip(pred, frames, coords):
            ...
            if str(args.preview_settings) == "True":
                cv2.imwrite(temp_directory + "preview.jpg", f)
                # Linux無法顯示圖片需註解
                # if not g_colab:
...
# args = parser.parse_args()
def run(**value):
    # args.face = value['face']
    # args.audio = value['audio']
    # args.outfile = value['outfile']
    args.pads = value['pads']
    args.checkpoint_path = value['checkpoint_path']
    args.out_height = value['out_height']
    args.fullres = value['fullres']
    args.quality = value['quality']
    args.mask_dilation = value['mask_dilation']
    args.mask_feathering = value['mask_feathering']
    args.nosmooth = value['nosmooth']
    args.debug_mask = value['debug_mask']
    args.preview_settings = value['preview_settings']
    args.mouth_tracking = value['mouth_tracking']
    # args.temp_directory = value['temp_directory']
    # args.detected_face_folder = value['detected_face_folder']
    do_load(args.checkpoint_path)
    main(value['face'], value['audio'], value['outfile'], value['temp_directory'], value['detected_face_folder'])
    
if __name__ == "__main__":
    args = parser.parse_args()
    do_load(args.checkpoint_path)
    main(args.face, args.audio, args.outfile, args.temp_directory, args.detected_face_folder)