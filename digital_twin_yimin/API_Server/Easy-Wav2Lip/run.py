...
def run(video_file, vocal_file, output_video_folder, output_video_name, temp_working_directory, temp_directory, detected_face_folder):
    parser = argparse.ArgumentParser(description='Easy-Wav2Lip main run file')

    parser.add_argument('-video_file', type=str, 
                        help='Input video file path', required=False, default=video_file)
    parser.add_argument('-vocal_file', type=str, 
                        help='Input audio file path', required=False, default=vocal_file)
    parser.add_argument('-output_file', type=str, 
                        help='Output video file path', required=False)
    ...
    temp_output = os.path.join(temp_working_directory, "temp", "output.mp4")
    temp_folder = os.path.join(temp_working_directory, "temp")
    ...
    while True:
        ...
        output_video = os.path.join(output_video_folder, output_video_name +  ".mp4")
        output_video = os.path.normpath(output_video)
        output_videofile = os.path.basename(output_video)

        # remove last outputs
        if os.path.exists(temp_working_directory + "temp"):
            shutil.rmtree(temp_working_directory + "temp")
        os.makedirs(temp_working_directory + "temp", exist_ok=True)
        ...
        last_detected_face = os.path.join(detected_face_folder, "last_detected_face.pkl")
        if os.path.isfile(detected_face_folder + "last_file.txt"):
            with open(detected_face_folder + "last_file.txt", "r") as file:
        ...
        cmd = [ 
            sys.executable, "get_video.py",
            "--face", temp_input_video,
            "--audio", temp_input_audio,
            "--outfile", temp_output,
            "--pads", str(-5), str(7), str(-3), str(-3),  # Pass pads as separate arguments
            ...
            "--detected_face_folder", detected_face_folder,
        ]
        ...
        if preview_settings:
            if os.path.isfile(os.path.join(temp_folder, "preview.jpg")):
                ...
                with open(detected_face_folder + "last_file.txt", "w") as f:
...
def set_information(username, video_path, audio_path):
    ...
    # 創建執行緒
    thread = threading.Thread(target=run, args=(input_video, input_audio, output_video_folder, output_video_name, temp_working_directory, temp_directory, detected_face_folder))
    # 啟動執行緒
    thread.start()
    
    # run(input_video, input_audio, output_video_folder, output_video_name, temp_working_directory, temp_directory, detected_face_folder)
    # run(input_video, input_audio, output_video_folder, output_video_name)